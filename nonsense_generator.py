"""Secure nonsense word generator."""
...
class NonsenseWordGenerator:
    """Generate pronounceable nonsense words using secure randomness."""
...
def main() -> None:
    ...
    print("Words for bucket names (6-12 chars):")
    words = gen.generate_batch(20, 6, 12)
    for i in range(0, len(words), 5):
        print("  ".join(f"{w:<15}" for w in words[i:i+5]))
    
    print("\nShort secure tokens (4-8 chars):")
    tokens = gen.generate_batch(20, 4, 8)
    for i in range(0, len(tokens), 5):
        print("  ".join(f"{t:<15}" for t in tokens[i:i+5]))
